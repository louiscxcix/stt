...
def speech_recognition_thread(recognizer):
    ...
    while True:
        try:
            ...
            # Ignore if speech is not understood
            pass
        except sr.RequestError:
            # Handle API request errors if necessary
            pass
        finally:
...
class AudioProcessor(AudioProcessorBase):
    ...
    def recv(self, frame: av.AudioFrame) -> av.AudioFrame:
        ...
        audio_data = sr.AudioData(
            raw_samples.tobytes(),
            frame.sample_rate,
            frame.layout.channels * 2  # 2 bytes per sample
        )
    ...
    def on_ended(self):
        ...
        st.session_state.full_transcript = final_transcript
...
# Update the placeholder with the live transcript
if webrtc_ctx.state.playing:
    transcript_placeholder.text_area("Your live transcript...", value=final_transcript, height=200)

# Once the stream stops, the full transcript is in the session state.
